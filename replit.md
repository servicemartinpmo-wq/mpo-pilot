...
- **Creator Lab** (`src/pages/CreatorLab.tsx`): Private backend at `/creator-lab` (passphrase: `apphia-creator`, localStorage key: `apphia_creator_unlocked`). Not linked in navigation — creator-only access. 5 tabs: (1) **AI Assistant** — prompt-based change engine using `processCreatorPrompt()` to analyze instructions and return structured change proposals with effort/impact; logs to `creator_prompts` and `org_memory`; (2) **Memory** — full CRUD on `org_memory` with category/type filters, stat boxes, add-entry form, resolve/delete per entry; (3) **Patterns** — auto-detected behavioral patterns from memory data using `detectPatterns()` + `generatePredictions()`; (4) **Customize** — accent color, font, density, banner theme, feature flags; (5) **Engine** — system chain stats + memory engine status.
