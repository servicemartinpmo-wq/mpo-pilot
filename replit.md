...
- **Memory Engine**: `src/lib/memoryEngine.ts` implements a lifelong persistent context memory system. Stores events, decisions, patterns, predictions, signals, insights, observations, AI changes, and milestones in the `org_memory` Supabase table (migration: `20260313000002_org_memory.sql`). Features: localStorage cache (5-min TTL), pattern recognition (frequency clusters, signal accumulation, tag co-occurrence, decision velocity, urgency patterns), prediction generation (finance review cycle, escalation risk, decision fatigue, resource crunch, compliance deadlines), and an `AIChangeResult` processor that analyzes creator prompts against keyword patterns to produce actionable change proposals. `loadMemorySnapshot()` returns entries + patterns + predictions in a single call.
- **Creator Lab** (`src/pages/CreatorLab.tsx`): Private backend at `/creator-lab` (passphrase: `apphia-creator`, localStorage key: `apphia_creator_unlocked`). Not linked in navigation — creator-only access. 6 tabs: (1) **Prompt Console** — terminal-style command input with `$` prefix, traffic-light dots, run/Tech-Ops buttons, run history; uses `processCreatorPrompt()` to produce structured change proposals; (2) **Access** — creator auto-gets Enterprise tier (shown as badge); Tier Editor (per-tier feature list accordion, add/remove/save features to `tier_definitions` table); Grant Tier Access form (email, tier, temp/permanent, expiry days, note) writing to `user_tier_grants`; Active Grants list with revoke; (3) **Memory** — full CRUD on `org_memory`; (4) **Patterns** — auto-detected patterns + predictions; (5) **Customize** — accent, font, density, banner theme, flags; (6) **Engine** — system stats + memory status.
- **Tier System** (`src/lib/tierSystem.ts`): Central tier management. Types: `TierId` = `free|solo|growth|command|enterprise`. `isCreatorUnlocked()` checks localStorage key. `useTierAccess(email)` hook resolves effective tier (creator override → user grant → profile tier → free). `fetchTierDefinitions()` / `saveTierDefinition()` manage DB-backed tier feature lists. `fetchTierGrants()` / `upsertTierGrant()` / `revokeTierGrant()` manage manual access grants. DB: `tier_definitions` (5 seed rows) and `user_tier_grants` tables (migration: `20260313000003_tier_access.sql`).
- **Tech-Ops Access Tab**: `/tech-ops` now has a third tab "Access" (Crown icon) showing: 5-tier overview grid with colors + prices, Active Tier Grants table with revoke action, and a deep-link button to Creator Lab for full editing.
- **Migration Hub** (`src/pages/MigrateHub.tsx`): Full import wizard at `/migrate` (also accessible via "Import / Migrate" in the sidebar Tools section). Supports Asana (CSV), Trello (JSON), ClickUp (CSV), Jira (CSV), Notion (CSV), and Generic CSV. 5-step wizard: tool selection → per-tool export instructions (with guide links + copy button) → drag-and-drop file upload → auto-detected field mapping (maps source columns to PMO fields: Title, Description, Assignee, Due Date, Priority, Status, Tags) → preview table (first 5 rows) → import to `action_items` in Supabase with auto-mapped status/priority normalization. Task type is tagged as `import:<tool-id>`.
- **Starred Nav / Shortcuts** (`src/hooks/useStarredNav.ts`): Hover over any sidebar nav item (Command or Tools section) to reveal a star icon. Click to star/unstar the page. Starred items appear in a persistent "Starred" section at the top of the sidebar (above the Command section), with an × remove button on hover. Stars are persisted in `localStorage` under key `pmo_starred_nav_v1` as `{ to, label }[]`. Works across all user modes; collapsed sidebar hides star controls and the Starred section.
- **AI Note Taker**: A tiered meeting note capture and summarization feature at `/note-taker`. Users can type or use browser Web Speech API for live voice-to-text transcription. An AI summary generator extracts key decisions, action items, and auto-titles from raw transcripts. Notes are saved with tags and viewable in a searchable history list. Free-tier users are limited to 3 notes with a usage counter and upgrade prompt (using existing `UpgradeBanner`); Solo+ tiers get unlimited access. Hook: `useNoteTaker.ts`. Data: `notes` table (migration `20260313000002_notes.sql`). Navigation: added to all 6 mode nav configs in `AppLayout.tsx`.
- **Data Services**: `supabaseDataService.ts` centralizes all CRUD operations with Supabase. Now includes `getOrgMemory`, `insertOrgMemory`, `deleteOrgMemory`, `resolveOrgMemory` for the memory engine.
